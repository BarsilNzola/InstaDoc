...
export default function ConnectWallet({ onConnect, compact = false }: ConnectWalletProps) {
  ...
  if (isConnected && address) {
    return (
      <div className={`flex items-center ${compact ? 'flex-col space-y-2' : 'flex-row space-x-3'}`}>
        <div className={`flex items-center space-x-2 ${compact ? 'justify-center' : ''}`}>
          <div className="w-2 h-2 rounded-full bg-green-500"></div>
          <span className={`font-medium ${compact ? 'text-sm' : ''}`} style={{ color: '#f2ead3' }}>
  ...
  return (
    <div className={`flex ${compact ? 'flex-col space-y-2' : 'flex-row space-x-3'} items-center`}>
      {connectors.map((c) => (
        <button
          key={c.id}
          onClick={() => connect({ connector: c })}
          disabled={isPending}
          className={`flex items-center justify-center space-x-2 transition-all duration-300 hover:shadow-md disabled:opacity-50 disabled:cursor-not-allowed ${
